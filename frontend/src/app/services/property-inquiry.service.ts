...
export class PropertyInquiryService {
  ...
  addInquiry(inquiry: PropertyInquiry): Observable<any> {
    return this.client.post(`${this.API_URL}/inquiries`, inquiry);
  }

  getAllInquiries(): Observable<any> {
    return this.client.get(`${this.API_URL}/inquiries`);
  }

  getInquiriesByUserId(userId: number): Observable<any> {
    return this.client.get(`${this.API_URL}/inquiries/user/${userId}`);
  }

  updateInquiry(inquiryId: number, inquiry: PropertyInquiry): Observable<any> {
    return this.client.put(`${this.API_URL}/inquiries/${inquiryId}`, inquiry);
  }

  deleteInquiry(inquiryId: number) {
    return this.client.delete(`${this.API_URL}/inquiries/${inquiryId}`);
  }
}